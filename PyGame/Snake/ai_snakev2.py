...
points_from_berry = 50 #sigmoid(0.1*currentGeneration-2) #Number of points we get for eating a berry, increases over time
points_from_moving = 1 #1-sigmoid(0.1*currentGeneration-2) #Number of points we get for avoiding walls, decreases over time
...
while not quitGame:
    ...
    if isPlaying:
        ...
        if currentPopulation == popSize - 1:
            ...
            print("%.2f"%maxberries)
            for g in gds:
                if g.berrycount >= 0.35*maxberries:
                    strong.append(g)
            ...
            if len(strong) < 0.10*popSize:
                ...
                while len(strong) < 0.10*popSize:
                    strong.append(GameData(SnakeNN(name=names[np.random.randint(0, len(names))] + ' ' + names[np.random.randint(0, len(names))])))
            else:
